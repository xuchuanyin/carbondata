...
public class CompactionResultSortProcessor extends AbstractResultProcessor {
   ...
   */
  private boolean[] isLongStringDimMapping;
  /**
  ...
  private void initSortDataRows() throws Exception {
    ...
    isLongStringDimMapping = new boolean[dimensions.size()];
    int i = 0;
    int j = 0;
    for (CarbonDimension dimension : dimensions) {
      ...
      if (dimension.getColumnSchema().isLongStringColumn()) {
        isLongStringDimMapping[j++] = true;
      }
      noDictionaryCount++;
    }
  ...
  private SortParameters createSortParameters() {
    return SortParameters
        .createSortParameters(carbonTable, carbonLoadModel.getDatabaseName(), tableName,
            ...
            carbonLoadModel.getTaskNo(), noDictionaryColMapping, isLongStringDimMapping, true);
  }
