...
public class IntermediateSortTempRow {
   ...
   * @param outNoDictNoSortAndLongString stores the no-dict & no-sort fields,
   *                                    including complex and long_string fields
   * @param outMeasures stores the measure fields
   * @param dataTypes data type for the measure
   */
  public void unpackNoSortFromBytes(int[] outDictNoSort, byte[][] outNoDictNoSortAndLongString,
      Object[] outMeasures, DataType[] dataTypes, int longStringDimCnt) {
    ...
    int noDictNoSortCnt = outNoDictNoSortAndLongString.length - longStringDimCnt;
    for (int i = 0; i < noDictNoSortCnt; i++) {
      short len = rowBuffer.getShort();
      byte[] bytes = new byte[len];
      rowBuffer.get(bytes);
      outNoDictNoSortAndLongString[i] = bytes;
    }

    // read long_string
    for (int i = 0; i < longStringDimCnt; i++) {
      int len = rowBuffer.getInt();
      byte[] bytes = new byte[len];
      rowBuffer.get(bytes);
      outNoDictNoSortAndLongString[i + noDictNoSortCnt] = bytes;
    }
