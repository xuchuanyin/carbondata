...
abstract class CarbonDDLSqlParser extends AbstractCarbonSparkSQLParser {
  ...
  def prepareTableModel(
    ...
    val (dims, msrs, noDictionaryDims, sortKeyDims, longStringColumns) = extractDimAndMsrFields(
  ...
  (Seq[Field], Seq[Field], Seq[String], Seq[String], Seq[String]) = {
    ...
    var longStringCols: Seq[String] = Seq[String]()
    ...
    if (tableProperties.get(CarbonCommonConstants.LONG_STRING_COLUMNS).isDefined) {
      ...
      longStringCols.foreach { longStringCol =>
        val exists = fields.exists(f => f.column.equalsIgnoreCase(longStringCol) &&
                                        DataTypes.STRING.getName.equalsIgnoreCase(f.dataType.get))
        if (!exists) {
          throw new MalformedCarbonCommandException(
            s"""
               |${CarbonCommonConstants.LONG_STRING_COLUMNS}: $longStringCol does not exist in table
    ...
    if (!sortKeyString.isEmpty) {
      ...
      sortKey.foreach { column =>
        if (!fields.exists(x => x.column.equalsIgnoreCase(column))) {
          ...
          if (longStringCols.exists(x => x.equalsIgnoreCase(column))) {
            throw new MalformedCarbonCommandException(
    ...
    if (sortKeyOption.isEmpty) {
      ...
      dimFields.foreach { field =>
        if (!isComplexDimDictionaryExclude(field.dataType.get) &&
            !longStringCols.contains(field.column)) {
    ...
    (dimFields.toSeq, msrFields, noDictionaryDims, sortKeyDims, longStringCols)
  }
